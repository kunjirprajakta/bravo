...
class Purchases extends CI_Controller
{
	...
	function index()
	{
        if(!$this->tank_auth->is_logged_in()){
            ...
            $data['view_details']	= base_url().'index.php/Purchases/view_details';
			$data['add_data']	= base_url().'index.php/Purchases/add_data';
            $data['delete_data']= base_url().'index.php/Purchases/delete_data';
            $data['edit_data']	= base_url().'index.php/Purchases/edit_data';
            $data['verify_data']	= base_url().'index.php/Purchases/verify_data';
            $data['edit_form']	= base_url().'index.php/Purchases/edit_form';

            $data['department_list'] = $this->Common_model->getAll("departments")->result_array();
            $data['purchases_list'] = $this->Common_model->getAll("purchases")->result_array();
    ...
    public function verify_data(){
        ...
        $verify=$this->Common_model->update("purchases",array('verified'=>'1'),array('id'=>$id));
        redirect(base_url('index.php/purchases'));
    }

    public function view_details(){
        $id=$this->input->post('id');
        $data['userinfo']	= $this->Common_model->getAll("users",array('id'=>$this->tank_auth->get_user_id()))->row_array();
        $data['view_details_info']=$this->Common_model->getAll("purchases",array('id'=>$id))->row_array();
        $this->load->view('common/header');
        $this->load->view('common/nav',$data);
...
$data['file']    = $file_upload['upload_data']['file_name'];
                    ...
                    $insert = $this->Common_model->insert("purchases",$data);
		            redirect(base_url('index.php/purchases'));
    ...
    public function edit_form(){
        $data=$this->input->post();
        $update=$this->Common_model->update("purchases",$data,array('id'=>$data['id']));
    ...
    public function edit_data(){
            ...
            $data['view_details']	= base_url().'index.php/Purchases/view_details';
            $data['edit_data']	= base_url().'index.php/Purchases/edit_data';
            $data['edit_data_info'] = $this->Common_model->getAll("purchases",array('id'=>$id))->row_array();
            $data['delete_data']= base_url().'index.php/Purchases/delete_data';
            $data['user_id']	= $this->tank_auth->get_user_id();
            $data['username']	= $this->tank_auth->get_username();
            $data['userinfo']	= $this->Common_model->getAll("users",array('id'=>$this->tank_auth->get_user_id()))->row_array();
            $data['skill_list'] = $this->Common_model->getAll("purchases")->result_array();
			$data['add_user']	= base_url().'index.php/Purchases/add_user';
            $data['delete_user']	= base_url().'index.php/Purchases/delete_department';
            $data['edit_user']	= base_url().'index.php/Purchases/edit_user';
            $data['department_list'] = $this->Common_model->getAll("departments")->result_array();
            $data['college_list'] = $this->Common_model->getAll("master_college")->result_array();
            $data['user_type_list'] = $this->Common_model->getAll("master_user_type")->result_array();
            $data['users_list'] = $this->Common_model->getAll("users")->result_array();
    ...
    public function delete_data(){
        $id=$this->input->post('id');
        $delete=$this->Common_model->delete("purchases",array('id'=>$id));
        redirect(base_url('index.php/purchases'));
    }
