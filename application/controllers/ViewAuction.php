...
<script type="text/javascript">
  $('.bidFunctn').click(function(){
	...
	function getOnline(){
		$users=$this->Common_model->getOnlineUsers()->result_array();
			foreach ($users as $val) {
			    $limit_time = date("Y-m-d H:i:s",time()+5*60);
				if(($val['loggedin']!="") && (strtotime($val['lastUpdate']>strtotime($limit_time) ))){ ?>
				    
					
					<li><a href="#"> <img height="10px" src="<?php base_url(); ?>/admin/assets/green.png"> <?php echo $val['username']; ?><br>
					<span style="color: #808080; font-size: 10px; text-align: center;"><?php echo (strtotime(date('Y-m-d H:i:s',time() + 1 * 60))); echo " minutes ago";  //$val['lastUpdate']; ?></span>
				</a>
	...
	function editlotsDetails($id)
	{
			...
			$data['lotlist'] = $this->Common_model->getAll("tb_lot",array("auction_id"=>$id))->result_array();
			$data['update_data']=base_url()."ViewAuction/update_lot_data";
			foreach($data['lotlist'] as $arrLotList){
				//echo $arrLotList['lot_description'];
			$data['lotData'] = $this->Common_model->getAllData("tb_bidding",array("lot_id"=>$arrLotList['id_lot']),"ASC","id_bidding")->result_array();
			foreach($data['lotData'] as $printData){
			...
			//$data['bidderList'] = $this->Common_model->getAll("tb_buyers")->result_array();
				...
				$this->load->view('common/header', $data);
				$this->load->view('common/nav');
				$this->load->view('editdetailTable',$data);
				$this->load->view('common/footer');
	...
	function update_lot_data(){
	    ...
	    $update=$this->Common_model->update("tb_bidding",$data,array('id_bidding'=>$data['id_bidding']));
